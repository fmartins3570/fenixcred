...
export default function QuestionnaireV2() {
  ...
  const startQuiz = useCallback(() => {
    ...
    trackEvent('InitiateCheckout', { content_name: 'Quiz Simulacao CLT V2', quiz_version: 'v2' }, eventId)
  ...
  const handleAnswer = useCallback((questionId, answerValue) => {
    ...
    if (question.type === 'tenure') {
      trackCustomEvent('PreQualTenure', {
        content_name: 'Quiz Simulacao CLT V2',
    ...
    if (questionIndex < QUESTIONS.length - 1) {
      ...
      trackCustomEvent('QuizQualified', {
        content_name: 'Quiz Simulacao CLT V2 - Pre-aprovado',
        quiz_version: 'v2',
        consignado_status: answerValue,
      }, qqEventId)
      sendServerEvent('QuizQualified', qqEventId, {
        page: '/simulacao-clt-v2',
      })
    }
  ...
  const handleCreditasClick = useCallback((product) => {
    trackCreditasRedirect(product, rejectionReason(rejectedAt), 'Quiz Simulacao CLT V2')
  }, [rejectedAt])

  useEffect(() => {
    if (screen === 'recovery') {
      trackCreditasRecoveryView(rejectionReason(rejectedAt), 'Quiz Simulacao CLT V2')
    }
  }, [screen, rejectedAt])
  ...
  const handleWhatsApp = useCallback(() => {
    ...
    else if (consignadoStatus === 'low') profileParts.push('Consignado ativo, parcela baixa (ate R$200)')
    ...
    const leadTag = `quiz-v2-${marginKey}-${bucket}-${refToken}`
    ...
    trackEvent('Contact', { content_name: 'Quiz Simulacao CLT V2 - WhatsApp', content_category: 'whatsapp', quiz_version: 'v2' }, contactEventId)
    ...
    sendServerEvent('Lead', leadEventId, {
      purposes: `quiz-v2:${marginKey}:${bucket}`,
    }, extraData)
        ...
        {screen === 'intro' && (
          <div className="sim-quiz-screen sim-quiz-intro" key="intro">
            ...
            <p className="sim-quiz-intro-text">
              Responda 4 perguntas rapidas e descubra se voce pode contratar
              o emprestimo consignado CLT com as melhores condicoes.
            </p>
            <button
          ...
          >
            ...
            {QUESTIONS[questionIndex].id === 'q4' && (
              <p className="sim-quiz-help">
                ...
                emprestimo consignado. Se nao tiver nenhum, sua margem
                provavelmente esta livre.
              </p>
            )}
          </div>
        )}
          ...
          <div className="sim-quiz-screen sim-quiz-approved" key="approved">
            ...
            <div className="sim-quiz-margin-helper">
              {!showSalaryHelper && estimatedMargin === 0 && (
                ...
                >
                  Nao sabe sua margem? Calcular pelo salario
                </button>
              )}
              ...
              {showSalaryHelper && estimatedMargin === 0 && (
                <div className="sim-quiz-margin-form">
                  ...
                  <span className="sim-quiz-margin-hint">
                    Usamos 35% do salario bruto (limite legal CLT).
                  </span>
                </div>
              )}
              ...
              {estimatedMargin > 0 && (
                <p className="sim-quiz-margin-result" aria-live="polite">
                  Margem estimada: <strong>{formatBRL(estimatedMargin)}/mes</strong>
                </p>
              )}
            </div>
        ...
        {screen === 'recovery' && (
          <div className="sim-quiz-screen sim-quiz-recovery" key="recovery">
            ...
            <p className="sim-recovery-text">
              {rejectedAt === 'q4'
                ? 'Com uma parcela alta de consignado ativo, sua margem pode estar esgotada. Mas voce pode conseguir credito usando seu imóvel ou veiculo como garantia, com taxas ainda menores.'
                : 'Voce pode conseguir credito usando seu imóvel ou veiculo como garantia, com taxas ainda menores e sem precisar de carteira assinada.'}
            </p>
